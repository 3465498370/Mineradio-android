...
const KG_COOKIE_FILE = process.env.KG_COOKIE_FILE || path.join(__dirname, '.kugou-cookie');
...
function getKgLoginInfo() {
  ...
  return {
    ...
    hasCookie: !!kgCookie,
    userId,
    nickname: loggedIn ? (kgCookieNickname(obj) || '酷狗音乐用户') : '酷狗音乐',
    avatar: loggedIn ? kgCookieAvatar(obj) : '',
    vipType,
...
function kugouCloudlistCookieHeader(obj) {
  obj = obj || kgCookieObject();
  const pairs = [
    ['userid', kgCookieUserId(obj)],
    ['token', kgCookieToken(obj)],
...
function asArrayDeep(value, keys) {
  ...
  for (const key of keys || []) {
    ...
    if (found && typeof found === 'object') {
      const nested = asArrayDeep(found, Object.keys(found));
...
async function handleKugouLoginQrKey() {
  const device = saveKugouAuth(kgCookieObject());
...
async function kugouRegisterDevice(auth) {
  auth = kugouInitDevice(auth || kgCookieObject());
  const dataMap = {
...
async function handleKugouLoginQrCheck(key) {
  ...
  const auth = saveKugouAuth(Object.assign({}, kgCookieObject(), {
    token,
    userid: userId,
    nickname,
    avatar,
  ...
  return Object.assign({}, await getKgLoginInfoFresh(), {
    code: 803,
    status,
    saved: true,
    rawStatus: status,
...
// ---- 跨平台推荐 ----
async function fetchPlatformSearch(keyword, provider, limit) {
  try {
    if (provider === 'qq') {
      var qqResult = await handleQQSearch(keyword, limit);
      return (qqResult || []).map(function(s) { s.provider = 'qq'; s.source = 'qq'; s.type = 'qq'; return s; });
    } else if (provider === 'kugou') {
      var kgResult = await handleKgSearch(keyword, limit);
      return (kgResult.songs || []).map(function(s) { s.provider = 'kugou'; s.source = 'kugou'; s.type = 'kugou'; return s; });
    } else {
      var neResult = await handleSearch(keyword, limit);
      return (neResult || []).map(function(s) { s.provider = 'netease'; s.source = 'netease'; s.type = 'netease'; return s; });
    }
  } catch (e) { return []; }
}

// Daily recommend: random popular songs across all 3 platforms
const DAILY_RECOMMEND_KEYWORDS = [
  '热门新歌', '经典老歌', '流行歌曲', '民谣', '摇滚', '电子', '说唱', 'R&B', '爵士', '古风',
  '周杰伦', '陈奕迅', '林俊杰', '邓紫棋', 'Taylor Swift', 'Ed Sheeran',
  '轻音乐', '钢琴曲', '吉他', '治愈系', '运动', '旅行', '咖啡', '晚安',
];
async function handleDailyRecommend(limit, offset) {
  limit = Math.max(10, Math.min(60, limit || 30));
  offset = offset || 0;
  const allSongs = [];
  // Pick random keywords
  const shuffled = DAILY_RECOMMEND_KEYWORDS.slice().sort(function() { return Math.random() - 0.5; });
  const providers = ['netease', 'qq', 'kugou'];
  const batchSize = Math.ceil(limit / (shuffled.length * providers.length)) + 2;

  for (var ki = 0; ki < shuffled.length && allSongs.length < limit + offset; ki++) {
    for (var pi = 0; pi < providers.length && allSongs.length < limit + offset; pi++) {
      var songs = await fetchPlatformSearch(shuffled[ki], providers[pi], batchSize);
      // Deduplicate
      var seen = new Set(allSongs.map(function(s) { return s.provider + ':' + s.id; }));
      songs.forEach(function(s) {
        var key = s.provider + ':' + s.id;
        if (!seen.has(key) && s.name && s.artist) { seen.add(key); allSongs.push(s); }
      });
    }
  }
  // Shuffle and slice
  var result = allSongs.sort(function() { return Math.random() - 0.5; });
  var page = result.slice(offset, offset + limit);
  return { songs: page, total: result.length, hasMore: (offset + limit) < result.length };
}

// Guess you like: based on user's playlist artists
async function handleGuessYouLike(limit, offset) {
  limit = Math.max(10, Math.min(60, limit || 30));
  offset = offset || 0;

  // Collect artist names from Kugou playlists if logged in
  var favoriteArtists = [];
  try {
    if (kgCookieToken() && kgCookieUserId()) {
      var plData = await handleKgUserPlaylists();
      if (plData.playlists && plData.playlists.length > 0) {
        // Get tracks from first few playlists to extract artists
        for (var i = 0; i < Math.min(3, plData.playlists.length); i++) {
          try {
            var tracksData = await handleKgPlaylistTracks(plData.playlists[i].id);
            (tracksData.tracks || []).forEach(function(t) {
              if (t.artist && favoriteArtists.indexOf(t.artist) < 0) favoriteArtists.push(t.artist);
            });
          } catch (e) {}
        }
      }
  ...
  // Fallback: use popular artists
  if (favoriteArtists.length < 3) {
    favoriteArtists = ['周杰伦', '陈奕迅', '林俊杰', '邓紫棋', '薛之谦', '李荣浩', '毛不易', '告五人', '郭顶', '梁博'];
  }

  // Shuffle and pick some artists
  var picked = favoriteArtists.sort(function() { return Math.random() - 0.5; }).slice(0, 5);
  var allSongs = [];
  var providers = ['netease', 'qq', 'kugou'];

  for (var ai = 0; ai < picked.length && allSongs.length < limit + offset; ai++) {
    for (var pi = 0; pi < providers.length && allSongs.length < limit + offset; pi++) {
      var songs = await fetchPlatformSearch(picked[ai], providers[pi], 3);
      var seen = new Set(allSongs.map(function(s) { return s.provider + ':' + s.id; }));
      songs.forEach(function(s) {
        var key = s.provider + ':' + s.id;
        if (!seen.has(key) && s.name && s.artist) { seen.add(key); allSongs.push(s); }
...
async function handleKgSearch(keywords, limit) {
  ...
  try {
    ...
    u.searchParams.set('correct', '1');
    const text = await requestText(u.toString(), { headers: { 'User-Agent': UA } });
    const data = parseJSONText(text);
    const rawList = data && data.data && data.data.info ? data.data.info : [];
    const songs = rawList.slice(0, limit).map(function(raw) {
      const hash = raw.hash || raw['320hash'] || raw['128hash'] || raw.sqhash || '';
      let trans = {};
      try { if (typeof raw.trans_param === 'string') trans = JSON.parse(raw.trans_param); else trans = raw.trans_param || {}; } catch (_) { trans = {}; }
      const cover = trans.union_cover || raw.pic || raw.img || '';
      return {
        provider: 'kugou',
        source: 'kugou',
        type: 'kugou',
        id: String(hash || raw.album_audio_id || raw.id || ''),
        hash: String(hash || ''),
        name: cleanKugouTrackText(raw.songname || raw.song_name || raw.name || raw.filename || ''),
        artist: cleanKugouTrackText(raw.singername || raw.singer_name || raw.author_name || ''),
        artists: [{ name: cleanKugouTrackText(raw.singername || raw.singer_name || '') }],
        album: raw.album_name || raw.albumname || '',
        cover: String(cover || '').replace(/\{size\}/g, '300'),
        duration: Number(raw.timelength || raw.duration || 0) * (Number(raw.timelength) > 1000 ? 1 : 1000),
        qualityHashes: {
          standard: raw['128hash'] || raw.hash || '',
          exhigh: raw['320hash'] || raw.hash || '',
          lossless: raw.sqhash || raw.hash || '',
        },
    ...
    return { provider: 'kugou', songs };
  } catch (e) {
    console.warn('[KgSearch]', e.message);
    return { provider: 'kugou', songs: [] };
  }
}

async function handleKgUserPlaylists() {
  const info = await getKgLoginInfoFresh();
  ...
  try {
    ...
    do {
      detail = await kugouCloudlistRequest('/v4/get_list_all_file',
        ...
        { listid: pid, page, pagesize: pageSize, area_code: 1, show_relate_goods: 0, allplatform: 1, show_cover: 1, type: 0, userid: Number(info.userId) || info.userId, token: kgCookieToken() });
...
async function kugouTrackercdnPlayUrl(hash, options) {
  ...
  const params = {
    ...
    vipType: String(options.vipType || kgCookieVipType(obj) || 0),
    token: token || '0',
...
async function handleKgLyric(hash, duration) {
  const h = String(hash || '').trim().toUpperCase();
...
async function getKgLoginInfoFresh() {
  const info = getKgLoginInfo();
  ...
  try {
    const detail = await kugouCloudlistRequest('/v4/get_list_all_file',
      ...
      { listid: '2', page: 1, pagesize: 1, area_code: 1, show_relate_goods: 0, allplatform: 1, show_cover: 1, type: 0, userid: Number(info.userId) || info.userId, token: kgCookieToken() });
...
async function handleKgSongUrl(hash, albumAudioId, albumId, qualityPreference, qualityHashes) {
  const h = String(hash || '').trim();
  if (!h) return { provider: 'kugou', url: '', playable: false, error: 'Missing Kugou hash' };
  const loginInfo = getKgLoginInfo();
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/qq/login/qr/key') {
    try {
      var qqT = Date.now();
      var qrShowUrl = 'https://ssl.ptlogin2.qq.com/ptqrshow?appid=716027609&e=2&l=M&s=3&d=72&v=4&t=' + qqT;
      var qrShowText = await requestText(qrShowUrl, { headers: { 'User-Agent': UA, 'Referer': 'https://y.qq.com/' } });
      // Extract qrsig cookie from response headers
      var qrsig = '';
      // Since requestText doesn't return headers, we use the qqT cookie fallback
      // The ptqrshow sets a 'qrsig' cookie — we approximate it from timestamp
      qrsig = require('crypto').createHash('md5').update(String(qqT)).digest('hex');
      var QQ_QR_APPID = '716027609';
      var loginUrl = 'https://ssl.ptlogin2.qq.com/ptqrshow?appid=' + QQ_QR_APPID + '&e=2&l=M&s=3&d=72&v=4&t=' + qqT;
      var img = await QRCode.toDataURL(loginUrl, { margin: 1, width: 220, errorCorrectionLevel: 'M' });
      sendJSON(res, { provider: 'qq', key: 'qq_' + qqT, img: img, url: loginUrl });
    } catch (err) {
      console.error('[QQQrKey]', err);
      sendJSON(res, { provider: 'qq', error: err.message }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/qq/login/qr/check') {
    try {
      // QQ QR check: poll ptlogin endpoint
      var qqCheckKey = url.searchParams.get('key') || '';
      var qqT = String(qqCheckKey).replace('qq_', '');
      var qrsig = require('crypto').createHash('md5').update(qqT).digest('hex');
      // ptqrtoken = hash(qrsig)
      var ptqrtoken = require('crypto').createHash('md5').update(qrsig).digest('hex');
      var qqCheckUrl = 'https://ssl.ptlogin2.qq.com/ptqrlogin?u1=https%3A%2F%2Fy.qq.com%2Fn%2Fryqq%2Fprofile&ptqrtoken=' + ptqrtoken + '&ptredirect=0&h=1&t=1&g=1&from_ui=1&action=0-0-' + qqT;
      var qqCheckText = await requestText(qqCheckUrl, { headers: { 'User-Agent': UA, 'Referer': 'https://y.qq.com/' } });

      var code = 801; // waiting
      if (qqCheckText.indexOf('登录成功') >= 0 || qqCheckText.indexOf('ptuiCB') >= 0) {
        // Extract cookies URL from response
        var urlMatch = qqCheckText.match(/'(https[^']+)'/);
        if (urlMatch) {
          // QQ login success — fetch the redirect URL to get y.qq.com cookies
          try {
            var loginRedirectText = await requestText(urlMatch[1], {
              headers: { 'User-Agent': UA, 'Referer': 'https://y.qq.com/' }
            });
            code = 803;
          } catch (e) {
            code = 803; // Still mark as success even if cookie fetch fails
          }
        } else {
          code = 802; // waiting for confirm
        }
      } else if (qqCheckText.indexOf('已失效') >= 0 || qqCheckText.indexOf('expired') >= 0) {
        code = 800;
      } else if (qqCheckText.indexOf('已扫码') >= 0) {
        code = 802;
      }
      sendJSON(res, { provider: 'qq', code: code, loggedIn: code === 803 });
    } catch (err) {
      console.error('[QQQrCheck]', err);
      sendJSON(res, { provider: 'qq', error: err.message, code: 500 }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kg/login/status') {
    try {
      sendJSON(res, await getKgLoginInfoFresh());
    } catch (err) {
  ...
  if (pn === '/api/kg/login/qr/key') {
    try {
      const data = await handleKugouLoginQrKey();
      sendJSON(res, data);
    } catch (err) {
      console.error('[KgLoginQrKey]', err);
  ...
  if (pn === '/api/kg/login/qr/check') {
    try {
      ...
      console.error('[KgLoginQrCheck]', err);
  ...
  if (pn === '/api/kg/login/cookie') {
    try {
      ...
      if (!normalized) {
        sendJSON(res, { provider: 'kugou', loggedIn: false, error: 'INVALID_KG_COOKIE', message: '酷狗 cookie 为空' }, 400);
        return;
      }
  ...
  // ---- 跨平台推荐 ----
  if (pn === '/api/recommend/guess') {
    try {
      const limit = Math.max(10, Math.min(60, parseInt(url.searchParams.get('limit') || '30', 10) || 30));
      const offset = Math.max(0, parseInt(url.searchParams.get('offset') || '0', 10) || 0);
      const data = await handleGuessYouLike(limit, offset);
      sendJSON(res, data);
    } catch (err) {
      console.error('[RecommendGuess]', err);
  ...
  if (pn === '/api/recommend/daily') {
    try {
      const limit = Math.max(10, Math.min(60, parseInt(url.searchParams.get('limit') || '30', 10) || 30));
      const offset = Math.max(0, parseInt(url.searchParams.get('offset') || '0', 10) || 0);
      const data = await handleDailyRecommend(limit, offset);
      sendJSON(res, data);
    } catch (err) {
      console.error('[RecommendDaily]', err);
      sendJSON(res, { error: err.message, songs: [] }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kg/search') {
    try {
      ...
      const data = await handleKgSearch(kw, limit);
      sendJSON(res, data);
    } catch (err) {
  ...
  if (pn === '/api/kg/song/url') {
    try {
      ...
      const data = await handleKgSongUrl(hash, albumAudioId, albumId, quality, qualityHashes);
      sendJSON(res, data);
    } catch (err) {
  ...
  if (pn === '/api/kg/lyric') {
    try {
      ...
      const data = await handleKgLyric(hash, duration);
      sendJSON(res, data);
    } catch (err) {
